...
{
    public partial class CntrlUsuFiltroStockMinimo : UserControl
    {
        public CntrlUsuFiltroStockMinimo()
        {
            InitializeComponent();
        }
        ...
        public string ConsultaSQL()
        {
            ...
            Consulta = "SELECT vp.ProductoID, vp.CodInmode, vp.Estado, vp.NomProd, vp.UnidMedida, vp.Marca, vp.NomGrupo, vp.NomSubGrupo, " +
                "vp.PCostoEmp, vp.PVentaMenorEmp, vp.PVentaMayorEmp, suc.SucursalID, suc.NomSuc, s.StockAlmacen, vp.StockMin FROM Vista_Productos vp " +
                "INNER JOIN Stock_Prod s ON s.ProductoID=vp.ProductoID INNER JOIN Sucursal suc ON suc.SucursalID=s.AlmacenID " +
                "WHERE s.StockAlmacen<=vp.StockMin AND vp.StockMin>0";
